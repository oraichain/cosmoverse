...
import { asset, assetInfo } from "../libs/utils";
...
describe("limit_order", () => {
  ...
  test("create orderbook pair", async () => {
    ...
    const queryPair_1 = await orderbook.orderBook({
    ...
    expect(queryPair_1).toEqual(expectedPair_1);
    ...
    const queryPair_2 = await orderbook.orderBook({
    ...
    expect(queryPair_2).toEqual(expectedPair_2);
  });

  it("submit native order", async () => {
    const orders: [Coin, Coin, OraiswapLimitOrderTypes.OrderDirection][] = [
  ...
  it("cancel native order", async () => {
    const cancelRes = await orderbook.cancelOrder({
      assetInfos: [assetInfo("orai"), assetInfo("usdt")],
      orderId: 1
    });

    expect(
      cancelRes.events.some((event) => event.attributes.some((attr) => attr.value === LISTEN_EVENTS.CANCEl_ORDER))
    ).toBe(true);

    try {
      const queryOrder1 = await orderbook.order({
        assetInfos: [assetInfo("orai"), assetInfo("usdt")],
        orderId: 1
      });
    } catch (ex) {
      expect(ex).toEqual(new Error("oraiswap_limit_order::orderbook::Order not found"));
    }
    ...
    const expectedAllorders = {
      orders: [
        ...
        },
        {
          ...
          bidder_addr: senderAddress,
          direction: "buy",
          filled_ask_amount: "630",
          filled_offer_amount: "107",
          order_id: 2
        }
      ]
    };
